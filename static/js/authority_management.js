...
var authorTab = $("#author_manage");

function showList(){

			  authorTab.bootstrapTable({
				  locale:'zh-CN',//中文支持
				  ...
				  columns: [
					  ...
					  {field: 'tool',title: '修改权限', align: 'center',width:'15%',
						  	  formatter: function (value,row,index){
						  	      ...
						  	      if (row.is_active) {
						  	         element = "<p>"
                                      + "市政管理" + "<input class='authorityList" + row.id +"' type='checkbox' name='checkbox" + row.id + "' value='1' />"
                                      + "农业管理" + "<input class='authorityList" + row.id +"' type='checkbox' name='checkbox" + row.id + "' value='2' /></p><p>"
                                      + "林业管理" + "<input class='authorityList" + row.id +"' type='checkbox' name='checkbox" + row.id + "' value='3' />"
                                      + "环境管理" + "<input class='authorityList" + row.id +"' type='checkbox' name='checkbox" + row.id + "' value='4' /></p><p>"
                                      + "道路管理" + "<input class='authorityList" + row.id +"' type='checkbox' name='checkbox" + row.id + "' value='5' />"
                                      + "沉降检测" + "<input class='authorityList" + row.id +"' type='checkbox' name='checkbox" + row.id + "' value='6' /></p><p>"
                                      ...
                                      + "</p>";
						  	         ...
						  	         element = "<p>"
                                      + "市政管理" + "<input class='authorityList" + row.id +"' type='checkbox' disabled='disabled' name='checkbox" + row.id + "' value='1' />"
                                      + "农业管理" + "<input class='authorityList" + row.id +"' type='checkbox' disabled='disabled' name='checkbox" + row.id + "' value='2' /></p><p>"
                                      + "林业管理" + "<input class='authorityList" + row.id +"' type='checkbox' disabled='disabled' name='checkbox" + row.id + "' value='3' />"
                                      + "环境管理" + "<input class='authorityList" + row.id +"' type='checkbox' disabled='disabled' name='checkbox" + row.id + "' value='4' /></p><p>"
                                      + "道路管理" + "<input class='authorityList" + row.id +"' type='checkbox' disabled='disabled' name='checkbox" + row.id + "' value='5' />"
                                      + "沉降检测" + "<input class='authorityList" + row.id +"' type='checkbox' disabled='disabled' name='checkbox" + row.id + "' value='6' /></p><p>"
    ...
    if(confirm(mes)==true){
        ...
        $.ajax({
            ...
            success:function(data){
                authorTab.bootstrapTable("updateRow",{
                    index:index,
                    row:data['user'],
                })
            },
